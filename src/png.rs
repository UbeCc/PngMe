// Copy the unit tests below and paste them at the bottom of your png.rs file.
// Write a Png struct with your implementation of PNG files.
// In your impl block, add a public constant called STANDARD_HEADER that has the 8 standard header bytes.
// Implement TryFrom<&[u8]> for your Png.
// Implement Display for your Png.
// Required methods:
...
impl std::convert::TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        if value.len() < 8 {
            return Err(Box::from("Invalid PNG"));
        }
        ...
        while index < value.len() {
            // first read the length, and then get the corresponding slice
            let length = u32::from_be_bytes([value[index], value[index + 1], value[index + 2], value[index + 3]]) as usize;
            let chunk = &value[index..index + length + 12];
...
impl Png {
    ...
    fn remove_first_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        match index {
            ...
            None => Err(Box::from("Chunk not found"))
        }
    }
